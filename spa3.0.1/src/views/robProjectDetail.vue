...
<template>
    <div>
        ...
        <div class="page" id="rob-project-detail-page" :style="{ height : (global.winHeight-2.943*16*global.winScale)+'px' }">
            ...
            <div class="counter"><counter v-if="itemData.startDate" :start="itemData.startDate" :end="itemData.endDate" :tip-text="tipText" @status-change="doCounterStatusChange"></counter></div>
            <router-link class="club" tag="div" :to="{ name : 'home' }"><div :style="{ backgroundImage : 'url('+global.clubLogoUrl+')' }"></div><div>{{ global.clubName }}</div><div></div></router-link>
            <div class="act-desc">
                <div class="node-pad-icon">活动说明</div>
                <div>
                    <div v-show="false"><div>活动时间</div><div>{{ itemData.startDate }}-{{ itemData.endDate }}</div></div>
                    <div>
            ...
            <div class="item-desc">
                ...
                <div>
                    ...
                    <div>
                        <div>项目说明</div>
                        <div><div v-html="itemData.description || '无'"></div></div>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    module.exports = {
        ...
        beforeRouteEnter : function(to,from,next){
            var query = to.query, global = Global.data, paramData = Util.sessionStorage('con-rob-project-param'), code = query.code;
        ...
        methods : {
            ...
            onBridgeReady : function(){
                var _this = this, global = _this.global, cashPayData = _this.cashPayData;
