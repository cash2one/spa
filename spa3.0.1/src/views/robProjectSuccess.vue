...
<template>
    <div class="page" id="rob-project-success-page">
        ...
        <router-link :to="{ name : 'home' }" class="club-info" tag="div"><div :style="{ backgroundImage : 'url('+global.clubLogoUrl+')' }"></div><div>{{ global.clubName }}</div><div></div></router-link>
        ...
        <div class="tech-list" v-if="clubObj.techs.length>0">
            <div class="title"><div>推荐技师</div><div>查看全部</div></div>
...
<script>
    ...
    module.exports = {
        ...
        beforeRouteEnter : function(to,from,next){
            var query = to.query, global = Global.data;
            if(!query.id){
        ...
        methods : {
            ...
            shareSetting : function(){
                var _this = this, global = _this.global, resData = _this.resData;
                Global.shareConfig({
