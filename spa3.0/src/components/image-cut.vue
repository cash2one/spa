...
<template>
    <div class="image-cut-container" :style="{ width : allW+'px', height : allH+'px' }"
         @touchstart="doTouchStartHandler($event)" @touchmove="doTouchMoveHandler($event)"
         @mousewheel="doMouseWheelHandler($event)">
    ...
    module.exports = {
        ...
        methods: {
            doGetBase64: function (option) {
                var _this = this
                var imgData = _this.getImageBase64(option.width, option.height)
            ...
            doTouchMoveHandler: function (e) {
                ...
                if (e.target != _this.rotateBtn) {
                    ...
                    if (_this.touchType == 1 && e.targetTouches.length == 1) {
                        ...
                        var zAdd = z / _this.pZ
                        var w = _this.imgW * zAdd
                        var h = _this.imgH * zAdd
            ...
            draw: function () {
                var _this = this
                var cx = _this.imgX + _this.imgW / 2
                var cy = _this.imgY + _this.imgH / 2
                var $ = _this.$
            ...
            getImageBase64: function (rw, rh) {
                ...
                var px = (_this.imgX - _this.vL) * zW
                var py = (_this.imgY - _this.vT) * zH
                var ow = _this.imgW * zW
                var oh = _this.imgH * zH
                var cx = px + ow / 2
                var cy = py + oh / 2
                var $$ = _this.$$
