...
<template>
    <div class="chat-input-wrap">
        <div class="input">
            <div contenteditable="true" ref="txtInput" @focus="onTextInputFocus()" @blur="onTextInputBlur()"
                 @click="onClickOfTextInput($event)" @input="onInputOfText()"></div>
        ...
        <div class="menu">
            ...
            <div class="expression" :class="{ active : showExpression }" @click="doClickExpressionBtn()"
                 ref="expressionBtn"></div>
            ...
            <div class="coupon" @click="doClickCouponBtn()" :class="{ active : showCoupons }"
                 v-show="talker.userType=='tech'">
                <ul>
                    <li v-for="coupon in coupons" @click="doClickCouponItem(coupon)">{{ coupon.actTitle }}</li>
                </ul>
            </div>
            <div class="dice" v-show="creditConfig.diceGameSwitch && talker.userType=='tech'"
                 @click="doClickDiceBtn()"></div>
            <div class="gift" v-show="creditConfig.creditSwitch" @click="doClickGiftBtn()"
                 :class="{ active : showGiftList }"></div>
        </div>
        ...
        <div class="gift-list" v-show="showGiftList">
            <div class="list" :style="{ width : gifts.length*5+'rem' }">
                <div class='gift-item' @click="doClickGiftItem(gift)" v-for="gift in gifts"
                     :class="{ active : currSelectGift == gift }">
                    <i></i>
    ...
    module.exports = {
        ...
        methods: {
            ...
            onTextInputFocus: function () {
                var _this = this
                var global = _this.global
                var txtInput = _this.$refs.txtInput
            ...
            onInputOfText: function () {
                ...
                var lastSelection = _this.lastSelection
                var sel = window.getSelection()
            ...
            doSelectedExp: function (event) {
                ...
                if (lastSelection.node && _this.isFocusInText(lastSelection.node)) {
                    ...
                    if (document.createRange) {
                        ...
                        if (cursorIndex) {
                            if (_this.isTxtInputFocus) {
                                _this.setCursorPosition(parentNode, cursorIndex)
                            } else {
            ...
            doChangeFileInput: function () {
                var _this = this
                var imgFile = _this.$refs.imgFile
            ...
            sendGift: function () {
                ...
                var talker = _this.talker
                var txtInput = _this.$refs.txtInput
                var currSelectGift = _this.currSelectGift
                var lastSelection = _this.lastSelection
                ...
                IM.sendTextMessage({
                    ...
                    _this.$http.get('../api/v2/credit/gift/send', {
                        params: {
