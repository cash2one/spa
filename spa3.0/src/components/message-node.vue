...
<template>
    <div class="messageNode" :class="messageCls" v-if="!(msgType=='diceGame' && game.status=='handled')">
        ...
        <template v-if="msgType=='diceGame'">
            <template v-if="game.status != 'over' && game.isMineInvite">
                ...
                <div :class='contentCls'>
                    ...
                    <div>
                        <div class='other'>
                            <div>{{ talker.name }}</div>
                            <div :class="game.status == 'request' ? 'wait' : 'reject'">{{ gameStatusObj[game.status]
                                }}<a class='cancel dice-game' v-if="game.status == 'request'"
                                     @click="doHandlerDiceGame('cancel')">取消</a></div>
                        </div>
                        <div class='dice'></div>
                        <div class='mine'>
                            <div>{{ im.name }}</div>
            ...
            <template v-if="game.status != 'over' && !game.isMineInvite">
                <div :style="{ backgroundImage : 'url('+talker.header+')' }"
                     @click="doClickTechHeader(talker.id)"></div>
                <div>
                    <div>
                        <div>{{ talker.name }}邀请您玩骰子<span v-if="game.status != 'request'">({{ gameStatusObj[game.status] }})</span>
                        </div>
            ...
            <template v-if="game.status == 'over'">
                ...
                <div><span>{{ game.isVictory ? "获取" : "消费" }}{{ msg.data }}积分，<a class='dice-game-tip'
                                                                                 @click="doClickMsgContent('dice-game-tip')">再玩一局</a></span>
                </div>
            </template>
        </template>
        ...
        <template
                v-if="msgType != 'paidCouponTip' && msgType != 'couponTip' && msgType !='diceGame' && msgType !='gift'">
            <div :style="{ backgroundImage : 'url('+( messageCls=='right' ? im.header : talker.header )+')' }"
                 @click="doClickTechHeader(messageCls=='right' ? im.id : talker.id)"></div>
            <div v-if="category=='text'" :class="contentCls" v-html="msgData"
                 @click="doClickMsgContent(contentCls)"></div>
            <div v-if="category=='pic'" :class="contentCls"><img :src="msg.url"
                                                                 :style="{ width : imgSize.w+'rem', height : imgSize.h+'rem' }"/>
            </div>
        </template>
    </div>
</template>
            ...
            _this.$nextTick(function () {
                ...
                if (game && game.needShow) {
                    setTimeout(function () {
                        ...
                        var userDiceImg = _this.$refs.userDiceImg
            ...
            doClickMsgContent: function (cls) {
                ...
                if (/begReward/.test(cls)) { // 点击求打赏消息，跳转到打赏页面
                    ...
                    if (!_this.global.userTel) {
                        ...
                        _this.$router.push({
                            name: 'paidCoupon',
                            query: {actId: _this.msg.actId, techCode: _this.msg.techCode, chanel: 'link'}
                        })
                    }
                }
            },
