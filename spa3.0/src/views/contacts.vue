...
<template>
    <div class="page message-list-page" id="contacts-page" v-show="!global.loading">
        <div class="page-title"><a class="back" @click="doClickPageBack()"></a>最近联系人
            <div class="edit-title" @click="doClickEditBtn()">{{ inEdit ? "完成" : "编辑"}}</div>
        </div>
        <div class="list" ref="listEle" :style="{ height : (global.winHeight-2.611*global.winScale*16)+'px' }"
            ...
            <div class="list-item" v-for="item in dataList" :key="item.friendUserId" @click="doClickRecord(item)">
                ...
                <div>
                    <div>{{ item.friendName}}<span
                            v-if="item.techNo"><span>[</span>{{ item.techNo }}<span>]</span></span></div>
                    <div>{{ item.msg }}</div>
    ...
    module.exports = {
        ...
        methods: {
            ...
            queryRecord: function (page) {
                ...
                var sessionList = _this.sessionList
                if (_this.isAddData) {
                    return
                }
