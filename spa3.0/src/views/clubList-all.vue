    ...
    <div class="page club-list-page" id="club-list-all-page" v-show="!global.loading" @click="doClickPage()">
        ...
        <div class="filter-title">
            <div :class="{ active : activeFilterType=='distance' }" @click="doActiveFilterType('distance',$event)">
                ...
                <div></div>
                <ul>
                    <li v-for="item in distanceList" :class="{ selected : selectedDistance==item.value }"
                        @click="doSelectedItem('Distance',item.value,$event)">{{ item.label }}
                    </li>
                </ul>
            </div>
            <div :class="{ active : activeFilterType=='sort' }" @click="doActiveFilterType('sort',$event)">
                ...
                <div></div>
                <ul>
                    <li v-for="item in sortFields" :class="{ selected : selectedSortField==item.value }"
                        @click="doSelectedItem('SortField',item.value,$event)">{{ item.label }}
                    </li>
                </ul>
            </div>
            <div :class="{ active : activeFilterType=='filter' }" @click="doActiveFilterType('filter',$event)">
                ...
                <div></div>
                <ul>
                    <li v-for="item in filters" :class="{ selected : selectedFilter==item.value }"
                        @click="doSelectedItem('Filter',item.value,$event)">{{ item.label }}
                    </li>
                </ul>
            </div>
        </div>
        <div class="list" ref="listEle" :style="{ height : (global.winHeight-7.507*global.winScale*16)+'px' }"
            ...
            <div class="finish-load-tip" :class="{ none : !showFinish }">
                <div>已经加载全部数据</div>
            </div>
    ...
    module.exports = {
        ...
        data: function () {
            return {
                ...
                selectedSortField: '',
                selectedFilter: '',
                distanceList: [{label: '附近会所', value: 30}, {label: '1000米', value: 1}, {
                    label: '2000米',
                    value: 2
                }, {label: '3000米', value: 3}, {label: '5000米', value: 5}, {label: '10千米', value: 10}, {
                    label: '全部会所',
                    value: 0
                }],
                sortFields: [{label: '默认排序', value: ''}, {label: '距离最近', value: 'distance'}, {
                    label: '人气最高',
                    value: 'uv'
                }],
        ...
        methods: {
            ...
            queryData: function (page) {
                ...
                _this.$http.get('../api/v2/club/all/clubs', {
                    params: {
                        page: page,
