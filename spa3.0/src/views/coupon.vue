    ...
    <div class="page" id="coupon-page" v-show="!global.loading" :style="{ height : global.winHeight+'px' }">
        ...
        <div class="list" ref="listEle" :style="{ height : (global.winHeight-2.611*global.winScale*16)+'px' }"
            ...
            <div class="list-item" v-for="singleClubData in dataList">
                ...
                <router-link v-for="item in singleClubData.list" class="item" :class="{ expire : item.isExpire }" :type="item.couponType"
                             :to="{ name : item.couponType=='paid' ? 'paidCouponDetail' : 'couponDetail', query : { userActId : item.userActId }}">
                    <i></i>
                    <div>{{ item.actTitle }}</div>
                    <div>{{ item.useType == 'money' ? ( item.actValue+'元现金券') : '' }}
                        <div v-html="item.consumeMoneyDescription"></div>
                    </div>
    ...
    module.exports = {
        ...
        methods: {
            ...
            queryRecord: function (page) {
                ...
                _this.$http.get(_this.getRecordsUrl, {
                    params: {
                        page: page,
