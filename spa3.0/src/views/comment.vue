    ...
    <div class="page" id="comment-page" v-show="!global.loading" :class="{ 'already-comment' : orderType != 0 }">
        ...
        <div class="comment-impression wrap">
            ...
            <div class="comment-labels">
                <div v-for="item in impressionList" :key="item.id" @click="doClickImpressionItem(item)"
                     :class="{ active : item.selected }">{{ item.tag }}
                </div>
            </div>
            <div class="comment-text" v-show="!(orderType == 1 && commentInputText.length == 0)">
                <textarea maxlength="200" @focus="onTextareaFocus()" @blur="onTextareaBlur()" v-model="commentInputText"
                          :readonly="orderType==1"></textarea>
                <div>200字以内哦~</div>
                <span :class="{ none : !showTextareaPlaceholder }">亲，点击这里，留句好评哦，么么哒~</span>
            </div>
            <div class="submit-btn" @click="doClickSubmitBtn()" :class="{ processing : inProcessing }">提交{{ inProcessing
                ? "中..." : "" }}
            </div>
        </div>
        <attention></attention>
...
<script>
    ...
    module.exports = {
        ...
        created: function () {
            ...
            global.loading = true
            _this.$http.get(_this.queryTechData, {
                params: {
                ...
                if (res.statusCode == 200) {
                    ...
                    var commentItems = _this.commentItems
                    ...
                    if (_this.orderType == 1) {
                        ...
                        for (i = 0; i < impressionArr.length; i++) {
                            ...
                            if (impressionLabelObj) {
                                ...
                                if (impressionObj) {
                                    Vue.set(_this.impressionList, impressionIndex, {
                    ...
                    _this.techId = techInfo.id
                    _this.techHeader = techInfo.avatarUrl || global.defaultHeader
                    _this.techName = techInfo.name || global.defaultTechName
                    _this.commentCount = techInfo.commentCount || 0
                    _this.techNo = techInfo.serialNo || '无'
        ...
        methods: {
                        ...
                        _this.$http.post(_this.submitUrl, {
                            ...
                            if (res.statusCode == 200) {
                                ...
                                if (global.userAgent.isWX) {
                                    _this.$router.push({
                                        name: 'techReward',
                                        query: {techId: _this.techId, commentId: res.respData || ''}
                                    })
                                }
            ...
            doClickImpressionItem: function (impressionItem) {
                var _this = this
                var k = 0
                var selectedList = _this.selectedImpression
                var impressionList = _this.impressionList
                if (_this.orderType != 0) return
