    ...
    <div>
        <div class="page" id="home-page" v-show="!global.loading">
            <div class="banner">
                ...
                <div class="logo" v-if="global.clubLogoUrl">
            ...
            <div class="recommend-tech" v-if="techs.length>0">
                <div>
                    <div></div>
                    推荐技师
                    <router-link :to="{ name : 'technicianList' }">全部技师</router-link>
                </div>
                <div>
                    <div>
                        <router-link v-for="tech in techs" :to="{ name : 'technicianDetail', query : { id : tech.id } }"
                                     :style="{ backgroundImage : 'url('+(tech.avatarUrl || global.defaultHeader)+')' }"
                                     class="tech">
                            <div>{{tech.name}}</div>
                        </router-link>
                    </div>
                ...
                <div class="paid-item" v-for="item in paidServiceItems">
                    ...
                    <div>
                        ...
                        <div>￥{{ item.amount }}<span>原价{{ item.price }}元</span>
                            <router-link :to="{ name : 'robProjectDetail' , query : { robProjectId : item.id }}">去抢购
                            </router-link>
                        </div>
            ...
            <div class="recommend-item">
                <div>
                    <div></div>
                    推荐项目
                    <router-link :to="{name : 'serviceList'}">全部项目</router-link>
                </div>
                <div>
                    <div class="item" v-for="item in serviceItems" @click="doClickServiceItem(item.id)">
                        ...
                        <div>
                            ...
                            <div v-show="item.price1">{{item.price1 |
                                itemPriceFormatter(item.duration1,item.durationUnit)}}
                            </div>
                        </div>
                    </div>
                </div>
