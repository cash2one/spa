    ...
    <div>
        <div class="page" id="order-detail-page" v-if="!global.loading">
            ...
            <div :style="{ height : (global.winHeight-2.611*global.winScale*16)+'px' }">
                ...
                <div>
                    <router-link
                            :style="{ backgroundImage : 'url('+(orderData.techAvatarUrl || global.defaultHeader)+')'}"
                            :to="{name:'chat',query:{techId:orderData.technicianId,clubId:orderData.clubId}}" tag="div"
                            v-if="orderData.technicianId"></router-link>
                    <div v-else
                         :style="{ backgroundImage : 'url('+(orderData.techAvatarUrl || global.defaultHeader)+')'}"></div>
                    <div>
                        <div>
                        ...
                        <div>{{orderData.technicianId?(orderData.techDescription || '这个技师很赖，没有填写个人简介...'):'到店选择技师'}}
                        </div>
                    </div>
                    <div :class="computeOrderStatus(orderData.status)">{{orderData.status |
                        OrderStatusFilter('name')}}
                    </div>
                </div>

                <div>
                    <div>预约项目<span>{{orderData.serviceItemName || '到店选择'}}</span></div>
                    <div>项目价格<span>{{orderData.servicePrie?(orderData.servicePrie | ItemPriceFormatter( orderData.serviceTime,orderData.serviceUnit)):'待定'}}</span>
                    </div>
                    <div>到店时间<span>{{orderData.appointTime | DateFormat}}</span><span>{{orderData.appointTime | DayFormat}}</span>
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="btn" v-if="orderData.status == 'submit'" @click="submitOrder()">
                        <div>确认预约</div>
                    </div>
                </div>
                    ...
                    <div>
                        ...
                        <div>
                            ...
                            <div :class="{ 'not-wx': !global.userAgent.isWX}">
                                <router-link tag="div"
                                             :to="{ name:'confirmOrder',query:{clubId:global.clubId,itemId:orderData.serviceItemId,techId:orderData.technicianId}}">
                                    再来一单
                                </router-link>
                                <router-link tag="div"
                                             :to="{ name:'techReward', query:{ techId: orderData.technicianId, backPublic:true, commentId:(clubInfo.comment?clubInfo.comment.id:'')} }">
                                    追加打赏
                                </router-link>
                            </div>
                        </div>
                ...
                <router-link tag="div"
                             v-show="orderData.status == 'complete' && orderData.commented == 'N' && orderData.technicianId"
                             :to="{ name:'comment',query:{ orderId:orderData.id,type:'order',techId:orderData.technicianId } }">
                    立即评论
                </router-link>
        ...
        methods: {
            ...
            submitOrder () { // 确认预约
                ...
                if (_this.orderData.technicianId) {
                    _this.$router.push({
                        name: 'chat',
                        query: {techId: _this.orderData.technicianId, clubId: _this.orderData.clubId}
                    })
                } else {
                    if (_this.global.clubTelephone.length == 0) {
                        Util.tipShow('暂无会所电话！')
