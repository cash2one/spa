...
<template>
    <div class="page" id="chat-page" v-show="!global.loading" :style="{ height : global.winHeight+'px' }">
        <div class="page-title"><a class="back" @click="doClickPageBack()"></a>{{ talker.name }}<span
                v-show="talker.userNo">[{{ talker.userNo }}]</span><a class="icon home"
                                                                      :class="{ once : talker.userType=='manager' }"
                                                                      @click="doClickHomeIcon()"></a>
            <router-link class="icon tech" v-show="talker.userType=='tech'" :to="{ name : 'technicianDetail', query : { id : talker.userId } }"></router-link>
        </div>
        <div class="order-tip" @click="doClickOrderTip()" v-show="talker.userType=='tech'">如果技师没有回应，那就立即预约吧！
            <div></div>
        </div>
    ...
    module.exports = {
        ...
        methods: {
            ...
            startDiceGame: function (amount) {
                ...
                var talker = _this.talker
                var im = _this.im
                _this.$http.get('../api/v2/credit/game/dice/submit', {
                    params: {
            ...
            doHandlerDiceGame: function (option, messageObj) {
                ...
                if ((+new Date()) - messageObj.time > _this.creditConfig.diceGameTimeout) { // 超时
                    ...
                    _this.$http.get('../api/v2/credit/game/dice/accept', {
                        params: {
