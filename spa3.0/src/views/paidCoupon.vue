...
<template>
    <div>
        <div class="page paid-coupon-page" id="paid-coupon-page" v-show="!global.loading"
             :style="{ height : (global.winHeight-6.966*global.winScale*16)+'px' }">
            ...
            <div class="coupon-info">
                <div>
                    <router-link
                            :style="{ backgroundImage : 'url('+(couponData.techs.avatarUrl || global.defaultHeader )+')' }"
                            :to="{ name : 'chat', query : { techId : couponData.techs.id, clubId : couponData.clubId }}"></router-link>
                    <div>{{ couponData.techs.name }}<span v-show="couponData.techs.serialNo">[<span>{{ couponData.techs.serialNo }}</span>]</span>
                    </div>
                </div>
                <div>
                    <div>
        ...
        <div class="paid-coupon-bottom-wrap">
            <div>
                <span @click="doClickChangeCount(0)">-</span><span>{{ payCount }}</span><span
                    @click="doClickChangeCount(1)">+</span>
                <div>共支付：￥<span>{{ payCount*couponData.actValue*100 | MoneyFormatter }}</span>元</div>
            </div>
            <div :class="{ processing : inPaid , downline : isDownLine }" @click="doClickPayBtn()">{{ isDownLine ? "已下线"
                : ( inPaid ? "购买中..." : "立即购买")}}
            </div>
        </div>
    </div>
    ...
    module.exports = {
        ...
        created: function () {
            ...
            if (!_this.actId || !_this.techCode) {
                ...
                _this.$http.get(_this.getDataUrl, {
                    params: {
                        actId: _this.actId,
                        userCode: '',
        ...
        methods: {
            ...
            doClickClubInfo: function () { // 点击会所
                var _this = this
                var global = _this.global
                var couponData = _this.couponData

                if (global.pageMode != 'club') {
                    _this.$router.push({name: 'home'})
                } else {
                    _this.$router.push({path: '/' + couponData.clubId + '/home'})
                }
            },
