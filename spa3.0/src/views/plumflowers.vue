    ...
    <div class="page" id="plumflowers-page">
        ...
        <div class="act-process">
            <div class="act-process-detail">
                ...
                <div><span>￥</span><span
                        class="price">{{ actDetail.price }}</span><span>/{{ actDetail.serviceTime }}分钟</span></div>
            </div>
        ...
        <div class="item act-result" v-show="showActResult">
            ...
            <div v-show="!showWinningNo">
                <ul>
                    <li><span>本次开奖将以<span class="lottery-date">{{ lotteryDate || '无' }}</span>福彩3D结果为参照，敬请期待！</span>
                    </li>
                </ul>
            </div>
            <div v-show="showWinningNo">
            ...
            <div>
                <ul>
                    <li v-for="item in actNos">
                        <div>
                            <div><span>手机<span>{{ item.phoneNum }}</span></span><span>{{ item.createdAt | DateFormatter('MM月dd日 hh:mm:ss') }}</span>
                            </div>
        ...
        <div class="item act-rules">
            ...
            <div>
                <ul>
                    ...
                    <li><span>当参与购买的达到{{ actDetail.price }}元，活动停止参与，并根据当天(20点之后则根据明天)福利彩票3D开奖号码计算出一个幸运号码，持有该号码的用户，即可获得奖励。</span>
                    </li>
                    <li><span>幸运号码计算办法：若开奖号码小于等于{{ actDetail.price }}即视作为幸运号码；若大于{{ actDetail.price }}则用该号码除以{{ actDetail.price }}余数作为幸运号码。</span>
                    </li>
        ...
        <div class="foot-btn">
            <div :class="isComplete ? 'next-time' : 'robed'" @click="doClickRobedBtn()">{{ isComplete ? '下次通知我' : '一元抢'
                }}
            </div>
            ...
            <div>
                ...
                <div class="join join-first" v-show="showJoinName=='first'">
                    ...
                    <div>
                        <div class="input-item">
                            <div>手机号码</div>
                            <div><input type="tel" placeholder="请输入您的手机号" v-model="telOfInputFirst" maxlength="11"
                                        v-tel-input/></div>
                        </div>
                        <div class="input-item">
                            <div>参与数量</div>
                            <div>
                                <div>
                                    <span class="reduce-count" @touchstart="doTouchStartReduce($event)"
                                          @touchmove="doTouchMoveReduce($event)"
                                          @touchend="doTouchEndReduce($event)"><span>-</span></span>
                                    <span class="times-count">{{ timesCount }}</span>
                                    <span class="plus-count" @touchstart="doTouchStartPlus($event)"
                                          @touchmove="doTouchMovePlus($event)"
                                          @touchend="doTouchEndPlus($event)">+</span>
                                </div>
                            </div>
                ...
                <div class="join join-notice" v-show="showJoinName=='notice'">
                    ...
                    <div v-show="showJoinNoticeTelInput">
                        <div class="input-item">
                            <div>手机号码</div>
                            <div><input type="tel" placeholder="请输入您的手机号" v-model="telOfInputNotice" maxlength="11"
                                        v-tel-input/></div>
                        </div>
                    </div>
                ...
                <div class="join join-search" v-show="showJoinName=='search'">
                    <div>输入手机号查看已购买</div>
                    <div>
                        <div><input type="tel" placeholder="请输入您的手机号" v-model="telOfInputSearch"/></div>
                    </div>
                    <div class="sure-btn" :class="{ disabled : !telOfInputSearchValid }"
                         @click="doClickJoinSearchBtn()">
                        <div>确定</div>
                    </div>
                </div>
            </div>
    ...
    module.exports = {
        ...
        methods: {
            ...
            onBridgeReady: function () {
                var _this = this
                var tradeReqData = _this.tradeReqData
                var payResData = _this.payResData
