...
<template>
    <div>
        ...
        <div class="page" id="technician-detail-page" v-show="!global.loading"
             :style="{ height : (global.winHeight-2.667*global.winScale*16)+'px' }">
            <div class="top">
                <div class="header">
            ...
            <div class="pics" v-show="techPics.length>0">
                <div>
                    <router-link v-for="pic in techPics" :style="{ backgroundImage : 'url('+pic.imageUrl+')' }"
                                 :to="{ name : 'technicianImg' , query : { id : techId , index : pic.orders }}"></router-link>
                </div>
            </div>
            ...
            <div class="service-item" v-show="serviceItems.length>0">
                <div class="title">选择项目
                    <div></div>
                </div>
                <div class="wrap">
                    <div class="item" v-for="service in serviceItems"
                         :class="{ selected : service.id == currSelectItem }" @click="doSelectServiceItem(service.id)">
                        <div></div>
                        ...
                        <div>
                            ...
                            <div v-show="service.price2">{{service.price2 |
                                itemPriceFormatter(service.duration2,service.durationUnitPlus)}}
                            </div>
                        </div>
                    </div>
                </div>
        ...
        <div class="club-coupon" :class="{ hide : paidCoupons.length==0 && ordinaryCoupons.length==0 }"
             @click="switchCouponListStatus(true)">
            <div></div>
            <span>抢优惠</span></div>
            ...
            <div>
                ...
                <div class="list">
                    ...
                    <div class="coupon-item ordinary" v-for="coupon in ordinaryCoupons">
                        <div>
                            <div>{{coupon.actTitle}}</div>
                            <div>{{(coupon.useType == 'money' ? (coupon.actValue+'元现金券，') : '') +
                                coupon.consumeMoneyDescription}}
                            </div>
                        </div>
                        <div>
                            <div>{{coupon.useTypeName}}</div>
                            <div>{{ (coupon.getFlag == 'already_get' ? '已领取' : (coupon.getFlag == 'finish_get' ? '已领完' :
                                '立即领取')) }}
                            </div>
                        </div>
                    </div>
