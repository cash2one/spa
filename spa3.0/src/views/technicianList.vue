    ...
    <div>
        <div class="page" id="technician-list-page" v-show="!global.loading">
            ...
            <div class="menu" :class="{ 'search-active': showSearchInput }">
                ...
                <div class="filter" @click="doClickShowServiceItemSelectArea()">
                    <div></div>
                    <div :class="{ active : currSelectItemName != '全部项目' }">{{currSelectItemName=="全部项目" ? "项目筛选" :
                        currSelectItemName }}
                    </div>
                </div>
            </div>

            <div class="tech-list" ref="listEle" :style="{ height : (global.winHeight-7.633*global.winScale*16)+'px' }"
        ...
        <div id="tech-list-select-area" class="pop-modal" :class="{ active : showServiceItemSelectArea }">
            <div>
                <div>
                    <div class="all">
                        <div :class="{ active : itemActiveId=='-1' }" @click="doClickServiceItem('-1','全部项目','')">全部项目
                        </div>
                    </div>
                    <div class="category" v-for="item in serviceItems">
                        <div>{{item.name}}</div>
                        <div>
                            <div v-for="subItem in item.serviceItems" :class="{ active : itemActiveId==subItem.id }"
    ...
    module.exports = {
        ...
        created: function () {
            ...
            if (pageData) {
                ...
                global.loading = true
                _this.$http.get(_this.getTechListUrl, {
                    params: {
        ...
        methods: {
            ...
            doHandlerTechListData: function (list) { // 获取列表数据之后的处理
                var _this = this
                var gTechList = _this.global.techList
                var gTechID = _this.global.techListID
