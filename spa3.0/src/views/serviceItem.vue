...
<template>
    <div>
        ...
        <div class="page" id="service-item-page" :style="{ height : (global.winHeight-3.333*global.winScale*16)+'px' }"
             v-if="!global.loading">
            <swipe class="profile-swipe" :show-indicators="false" :continuous="false" :auto="24*60*60*1000"
                   :curr-index="swipeInitIndex" v-if="dataList.length>0">
                <swipe-item v-for="item in dataList" :key="item.id">
                    <div class="service-item-top">
                        <div class="img"
                             :style="{ backgroundImage : 'url('+(item.imageUrl || global.defaultServiceItemImgUrl)+')' }"></div>
                        <div>
                            <div>{{item.name}}</div>
                            ...
                            <div v-show="item.pricePlus">加钟{{item.pricePlus |
                                itemPriceFormatter(item.durationPlus,item.durationUnitPlus)}}
                            </div>
                        </div>
                    </div>
    ...
    module.exports = {
        ...
        methods: {
            queryItemData: function (itemId, direction) {
                var _this = this
                _this.$http.get(_this.queryDataUrl, {
                    params: {
                        top: 0,
                        index: direction,
                        itemId: itemId,
                    ...
                    if (res.statusCode == 200) {
                        ...
                        if (direction == 0) { // 初始加载数据
                            ...
                            if (data.telephone) {
                                _this.telephone = data.telephone.split(',')
                            }
                        }
