...
<template>
    <div class="page" id="promotions-page" v-show="!global.loading">
        ...
        <div v-for="coupon in coupons"
             :class="[coupon['getFlag'], 'money', coupon['useType']=='money' ? '' : 'coupon' ]">
            <div>
                <div><span v-if="coupon['useType'] == 'money'">￥</span>{{coupon['useType'] == 'money' ?
                    coupon['actValue'] : coupon['actTitle'] }}
                </div>
                <span v-show="coupon['useType']!='money' || coupon['consumeMoney']!=0">{{coupon['useType']=='money' ? coupon['consumeMoneyDescription'] : coupon['actValue']}}<span
                        v-if="coupon['useType']!='money'">元&nbsp;&nbsp;&nbsp;&nbsp;原价：<span>{{coupon['consumeMoney']}}元</span></span></span>
                <div>
                    <div>{{coupon['useTypeName']}}</div>
                    <div>{{coupon['getFlag']=='already_get' ? '已领取' : (coupon['getFlag']=='finish_get' ? '已领取完' :
                        '立即领取') }}
                    </div>
                    <div></div>
                    <div></div>
        ...
        <router-link v-for="act in activities" class="activity" :style="{ backgroundImage : 'url('+act.actLogoUrl+')' }"
