require(["css!../../compressed/css/page/"+$$.rootVm.page+".css?"+$$.rootVm.currTime,"daterangepicker","!domReady"],function(){
    ...
    var vm = avalon.define({
        ...
        couponFeeStr : "",
        detailObj : {////详情对象
            actValue : "", consumeMoney : "", couponPeriod : "", actContent : "", incomeType : "",commission:0
        },
        editObj : {/////编辑对象
            actId : "", actValue : "", consumeMoney : "", couponPeriod : "30", actContent : "", incomeType : "tech",commission:''
        },
        dayOptionArr : [
            {"name":"1天","value":"1"},
            {"name":"2天","value":"2"},
        ...
        doEditViewCoupon : function(index){////查看或者编辑
            if(this.innerHTML == "查看"){
                ...
                var couponPeriodArr = vm.dataList[index].couponPeriod.match(/\d{1,2}/g);
                //$('#editPeriod').val(couponPeriodArr[couponPeriodArr.length-1]);
                $('#longAfterReceive').val(couponPeriodArr.length>1?couponPeriodArr[0]:0);
                vm.editObj.couponPeriod = parseInt(couponPeriodArr[couponPeriodArr.length-1])+"";
                vm.editObj.actContent = vm.dataList[index].actContent;
                vm.editObj.incomeType = vm.dataList[index].incomeType;
                vm.editObj.commission = vm.dataList[index].commission;
    ...
    function checkForm(){
        ...
        if(!couponCommission.val()){
            couponEditModal.showTip("请输入技师提成！");
            editActConsumeMoney.focus();
            return false;
        }

        ////////校验购买金额是否小于手续费
        if(parseInt(editActValue.val()) < parseFloat(couponCommission.val())){
            couponEditModal.showTip("购买金额不能小于技师提成！");
            return false;
        }

        if(editActContent.html() && editActContent.text().length>2000){
    ...
    ///////限制输入
    $("#editActValue,#editActConsumeMoney,#couponCommission").on("input",function(){
        if (/\D/.test(this.value)) {
            this.value = this.value.replace(/\D/g, '');
        }
        if(this.value.length>5) this.value = this.value.substr(0,5);
    });
    /*couponCommission.on("input",function(){
