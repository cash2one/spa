require(["css!../../compressed/css/page/"+$$.rootVm.page+".css?"+$$.rootVm.currTime,"colorbox","qrcode","!domReady"],function(){
    ...
    confirmModal = new Modal($("#confirmModal"),{
        doClickOkBtn : function(){
            ...
            else if(vm.opeType == "delete" || vm.opeType == "quit"){
                $.ajax({
                    url : "tech/delete/"+pageParam.id,
                    data : { operation : vm.opeType },
                    success : function(res){
                        confirmModal.close();
                        if(res.statusCode==200){
                            msgAlert(res.message,true);
                            if(vm.opeType == "delete"){
                                location.href="#!/techList";
                            }else {
                                location.href="#!/historyTech";
                            }
                        }
                        else{
            ...
            success : function(res){
                if(res && res.data){
                    if(refreshServiceItem===true){
                        ...
                        if(res.data.name.length>20) res.data.name = res.data.name.substr(0,20)+"...";
    ...
    //===== 生成二维码 ====
    $('#techQrCode>div.mini-qr-code').qrcode({
        width:70,
        height:70,
        text:location.origin+(location.pathname.split(';')[0]+'spa2/?club='+$$.clubId+'&clubsource=tech_qrcode#technicianDetail&id='+ pageParam.id),
        roundBlank:false         //四周是否留白
    });
    $('#techQrCode>div.max-qr-code>div:first-child').qrcode({
        width:430,
        height:430,
        text:location.origin+(location.pathname.split(';')[0]+'spa2/?club='+ $$.clubId+'&clubsource=tech_qrcode#technicianDetail&id='+ pageParam.id),
        roundBlank:true         //四周是否留白
    });
    $('#commentQrCode>div.mini-qr-code').qrcode({
        width:70,
        height:70,
        text:location.origin+(location.pathname.split(';')[0]+'spa2/?club='+$$.clubId+'&clubsource=tech_qrcode#technicianDetail&id='+ pageParam.id + '/comment&type=tech&isScan=1&techId=' + pageParam.id),
        roundBlank:false         //四周是否留白
    });
    $('#commentQrCode>div.max-qr-code>div:first-child').qrcode({
        width:430,
        height:430,
        text:location.origin+(location.pathname.split(';')[0]+'spa2/?club='+$$.clubId+'&clubsource=tech_qrcode#technicianDetail&id='+ pageParam.id + '/comment&type=tech&isScan=1&techId=' + pageParam.id),
        roundBlank:true         //四周是否留白
