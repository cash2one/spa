<style>
    @import '../styles/page/luckyWheel.css';
</style>
<template>
    <div class="page" id="lucky-wheel-page">
        <template>
            ...
            <div class="counter-desc ani" v-show="!global.loading && !loadError"><div>剩余<span>{{ currLotteryCount }}</span>次抽奖机会</div></div>
            ...
            <div class="records ani"><router-link :to="{name: 'lucky', query: {back: 1}}">我的抽奖纪录</router-link></div>
            <div class="list-wrap winning-list ani" v-show="!global.loading && !loadError">
                ...
                <ul>
                    ...
                    <li v-if="recordList.length > 0" v-for="item in recordList"><div><span>恭喜</span>{{ item.userName }}</div><div>获得{{ item.prizeName }}</div></li>
                </ul>
            </div>
