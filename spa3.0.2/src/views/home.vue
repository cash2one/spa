    ...
    <div>
        <div class="page" id="home-page" v-show="!global.loading">
            ...
            <div class="recommend-item">
                <div>
                    <div></div>推荐项目<router-link :to="{name : 'serviceList'}">全部项目</router-link>
                </div>
                <div>
...
<script>
    ...
    module.exports = {
        ...
        mounted: function () {
            ...
            // 请求优惠券或者一元夺美女数据
            that.$http.get('../api/v2/club/{clubId}/top_popup/data', {params: {clubId: global.clubId}}).then(function (popRes) {
                popRes = popRes.body
                if (popRes.statusCode == 200) {
                    popRes = popRes.respData
                    if (popRes.type == 'coupon') {
                    } else if (popRes.type == 'plumFlower') {
                        that.plumShareUrl = popRes.shareUrl
