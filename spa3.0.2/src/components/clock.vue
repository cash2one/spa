...
<template>
    <div class="clock-wrap">
        ...
        <label>:</label>
        <b>{{ sec }}</b>
        <label v-show="showMilliSecond">:</label>
        <b>{{ milliSec }}</b>
    </div>
</template>
...
<script>
    module.exports = {
        ...
        watch: {
            status: function (curVal) { // 监控状态的改变
                if (curVal != '') {
        ...
        computed: {
            startTime: function () { // 开始时间
                return new Date(this.start.replace(/-/g, '/')).getTime()
            },
            endTime: function () { // 结束时间
