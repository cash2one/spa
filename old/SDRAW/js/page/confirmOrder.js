        ...
        $.ajax({
            ...
            success : function(data){
                ...
                var i,
                  ...
                  arriveTime = $('#content>div:nth-of-type(4)>div:nth-of-type(2)>span:nth-of-type(2)')[0],
                  millsecondsOfOneDay = 24*60*60*1000,
                  timePicker,
                  isSubmit=false,
                  footer=$('#footer>div'),
                  contactName = $("#content>div:nth-of-type(5)>div:nth-of-type(1)>input"),
                  timePickerInitTag = false;

                data = data.respData;
                downPayment = data.downPayment || 0;
                payAppointment = data.payAppointment;
                /////////////////////////技师信息
                if(data["tech"]){
                ...
                for(var k = 1;k<4;k++){
                    ...
                    $(dayListDiv[k-1]).Attr('data-index',k);
                    fdates.push(fObj);
                }
                showTimePicker(0);
                if(!isActive){
                        ...
                        if(!oldOrderId){
                            ...
                            if(payAppointment&&payAppointment == 'on'&&downPayment>0){
                                ...
                                if(!$.$.userToken){//未登录
                                    ...
                                    footer.Text('预约中...');
                                    doPayCoupon($('.day-list>div.active').Attr('data-index') - 1
                                ...
                                //setTimeout(function () {
                                    $.ajax({
                                        ...
                                        data:{
                                            ...
                                            dateDay: $('.day-list>div.active').Attr('data-index') - 1 ,
        ...
        timesData[index]['time'].forEach(function (v, i) {
            ...
            if(v.timeStr){
                if(v.status === 'Y' && !v.tmpFlag){
                    htmlStr.push('<div ix="'+i+'" class="'+_class+'"><span '+(v.tmpFlag|| v.status==='N'?'':'style="display:block;"')+'>'+ v.timeStr +'</span><span '+(v.tmpFlag|| v.status==='N'?"":'style="display:none;"')+'>&#45;&#45;:&#45;&#45;</span></div>')
                }else{
                    htmlStr.push('<div ix="'+i+'" class="'+_class+'"><span >'+ v.timeStr +'</span><span >&#45;&#45;:&#45;&#45;</span></div>')
                }
            }
        });
