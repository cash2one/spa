    ...
    $.ajax({
        ...
        success: function (result) {
            ...
            if(result.statusCode == '200'){
                ...
                $('#explainList').Html('<li>1、'+result.memberTypeName+'会员'+(result.discount/100 >= 10?'无折扣享受；':('享受'+(result.discount/100).toFixed(2).replace(/0*$/g,'').replace(/\.$/g ,'')+'折优惠；'))+'</li>\
                                    <li>2、请在会员结账时主动提供会员二维码或者会员卡号或者注册手机号；</li>\
                                    <li>3、解释权归'+result.clubName+'所有；</li>');
            }else{
