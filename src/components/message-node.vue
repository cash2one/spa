...
<template>
    <div class="messageNode" :class="messageCls" v-if="!(msgType=='diceGame' && game.status=='handled')">
        <template v-if="msgType=='paidCouponTip' || msgType == 'couponTip'">
            <div>{{ msg.time | MsgTimeFormatter }}</div>
        ...
        <template v-if="msgType=='diceGame' && game.status != 'over' && !game.isMineInvite">
            ...
            <div>
                ...
                <div :class="{ disabled : game.status != 'request' }">
                    <div class='reject'>拒绝</div><div class='accept'>接受</div>
                </div>
            </div>
        </template>
        <template v-if="msgType=='diceGame' && game.status == 'over'">
            <span>{{ msg.time | MsgTimeFormatter }}</span>
            <div :style="{ backgroundImage : 'url('+im.header+')' }"></div>
            <div :class='contentCls'>
            ...
            <div><span>{{ game.isVictory ? "获取" : "消费" }}{{ msg.data }}积分，<a class='dice-game-tip'>再玩一局</a></span></div>
        </template>
        ...
        <template v-if="msgType==''">
            ...
            <div v-if="category=='text'" :class="contentCls" v-html="msgData"></div>
        ...
        computed : {
            ...
            game : function(){
                ...
                if(_this.msgType == "diceGame"){
                    ...
                    obj.isMineInvite = obj.invite == _this.im.id;
                    if(obj.status == "over"){
                        var gameResult = (msg.gameResult || msg.ext.gameResult).split(":");
                        obj.isVictory = (obj.isMineInvite ? (gameResult[0] > gameResult[1]) : (gameResult[0] < gameResult[1]));
