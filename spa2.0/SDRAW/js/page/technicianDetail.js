    ...
    function initPage() {
        ...
        $.ajax({
            ...
            success: function (data) {
                ...
                //内网对接，忙时显示下钟倒计时
                var currTime = +new Date(), endTimeProject;

                function calcTime() {
                    var round = Math.round, floor = Math.floor, millis,
                        sec, min, hour
                    ...
                    millis = endTimeProject - currTime;
                    endTimeProject = millis / 1000;
                    if (endTimeProject > 0) {
                        sec = floor(endTimeProject % 60);
                        min = floor(endTimeProject / 60 % 60);
                        hour = floor(endTimeProject / 3600 % 24);
                        timeData = [floor(hour / 10), hour % 10, floor(min / 10), min % 10, floor(sec / 10), sec % 10];
                        //timeData = [0,1,1,0,0,1];
                        ...
                        function startToggle() {
                            var tmpArr = [];
                            if (timeData[5] > 0) {
                                timeData = [timeData[0], timeData[1], timeData[2], timeData[3], timeData[4], timeData[5] - 1];
                                tmpArr = tmpArr.concat([5]);
                            } else if (timeData[4] > 0) {
                                timeData = [timeData[0], timeData[1], timeData[2], timeData[3], timeData[4] - 1, 9];
                                tmpArr = tmpArr.concat([4, 5]);
                            } else if (timeData[3] > 0) {
                                timeData = [timeData[0], timeData[1], timeData[2], timeData[3] - 1, 5, 9];
                                tmpArr = tmpArr.concat([3, 4, 5]);
                            } else if (timeData[2] > 0) {
                                timeData = [timeData[0], timeData[1], timeData[2] - 1, 9, 5, 9];
                                tmpArr = tmpArr.concat([2, 3, 4, 5]);
                            } else if (timeData[1] > 0) {
                                timeData = [timeData[0], timeData[1] - 1, 5, 9, 5, 9];
                                tmpArr = tmpArr.concat([1, 2, 3, 4, 5]);
                            } else if (timeData[0] > 0) {
                                timeData = [timeData[0] - 1, 9, 5, 9, 5, 9];
                                tmpArr = tmpArr.concat([0, 1, 2, 3, 4, 5]);
                            } else {
                                return true;
                            }
                            changeNums(tmpArr);
                ...
                if (isCrossInner && data.info.status == 'busy' && data.info.endTime) {
                    endTimeProject = +new Date(data.info.endTime.replace(/-/g, '/'));
                    if (endTimeProject - currTime > 0) {
                        $('#content>div:nth-of-type(1)').Class('cross-inner');
                        calcTime();
                    }
                }
                /*************************************定义常用变量*************************************/
                ...
                $('#content>div:nth-of-type(1)>div:nth-of-type(1)>div:nth-of-type(2)>div:nth-of-type(3)').Text({
                    free: '闲',
                    busy: '忙',
                    rest: '休'
                }[data['info']['status']]);
                ...
                $('#content>div:nth-of-type(4)')[0].onclick = function () {
                    if ($.$.clubID) {
                        ...
                        location.href = location.origin + location.pathname + "?club=" + clubId + "#technicianList" + ($.$.visitChannel == '9358' ? '&clubsource=9358' : '');
                    }
                };
                //点击收藏
                ...
                footer.Click(function (e, item) {
                    if (footer.ClassHave('active')) {
                        if (data.appointType == 'phone') {
                            if ($.login()) return;
                            if (data['telephone'] == '') $.tipShow('暂无预约电话');
                            else telDetail.Class('active');
                        } else if (data.appointType == 'paid' || data.appointType == 'paid_full') {
                            if (!$.$.ua.isWX) {
                                if ($.checkAccessMenu('confirmOrder')) {
                                    $.tipShow('【此会所需支付预约，请在微信中打开】');
                                }
                            } else {
                                if ((isCrossInner || data.appointType == 'paid_full') && !selectedItem) {
                                    $.tipShow('必须选择一个服务项目', 4000);
                                    return;
                                }
                        ...
                        } else if(data.appointType == 'appoint') {
                            $.login('confirmOrder&techId=' + techId + (selectedItem ? "&itemId=" + selectedItem : '') + ($.$.visitChannel == "9358" ? "&clubId=" + data["info"]["clubId"] : ""), false, true, true);
                        } else {
                            $.tipShow('此会所不支持线上预约。');
                        }
                ...
                if ($.$.ua.isWX) {
                    $.X5Config({
                        ...
                        "desc": "点我聊聊，更多优惠，更好服务！",
                        "link": location.href.split("?")[0] + "?club=" + data["info"]["clubId"] + "#technicianDetail&visitChannel=9358&isNeedFollow=true&id=" + $.param('id'),
        ...
        //=== 获取技师图片 ===
        var visitSource = $.$.clubSource;
        if(!visitSource){
            visitSource = $.getUrlParam('clubsource') || "link";
            if(visitSource && /wifi/.test(visitSource)){
                visitSource = "wifi";
            }
        }
        $.ajax({
            url: '../api/v2/club/tech/albums/' + techId,
            isReplaceUrl: true,
            data:{
                source: visitSource
            },
            success: function (result) {
                if (result.statusCode == 200) {
                    ...
                    var str = '', str1 = '', str2 = '';
                    for (var i = 0, dataLength = result.length; i < dataLength; i++) {
                        if (!result[i]['imageUrl']) continue;
                        str += '<div style="background-image: url(\'' + result[i]['imageUrl'] + '\')"></div>';
                        str1 += '<img imageCache="' + result[i]['bigImageUrl'] + '"/>';
                        str2 += '<span></span>';
                    }
                    ...
                    if (str == '')
                        $('#content>div:nth-of-type(2)').Hide();
                    else $('#content>div:nth-of-type(2)').Show();
                    ...
                    if (result.statusCode == 200) {
                        ...
                        result.forEach(function (v) {
                            if (v.couponType == 'paid') {
                              ...
                              <div>\
                                <div>点钟券</div>\
                                <div class="sure-btn" data-act-id="' + v.actId + '" data-act-value="' + v.actValue + '" data-act-title="' + v.actTitle + '">立即购买</div>\
                              </div>\
                            </div>');
                            } else {
