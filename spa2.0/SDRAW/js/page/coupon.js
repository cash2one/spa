(function () {
    ...
    function generateItemHtml(itemData) {
        ...
        if(itemData["credits"] && itemData["credits"] - 0 > 0){
            itemData["consumeMoneyDescription"] = itemData["credits"] + " 积分";
        }
        ...
        /*if(isPaidServiceItem && !isExpire){
            ...
            if(Math.floor(endDate/86400000)>0){
                ...
                endDate = '<span>'+ Math.floor(endDate/3600000) + '</span>时';
            }else{
                endDate = '<span>'+ Math.floor(endDate/60000) + '</span> 分';
            }
            htmlStr += '<div>'+endDate+'</div>';
        }*/
        htmlStr += '</div>';
        return htmlStr;
    }
