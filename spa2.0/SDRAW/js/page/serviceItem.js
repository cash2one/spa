(function(){
    ...
    function initAppointment(data){
        ...
        if(data.appointment == 'N' && data.phoneAppointment == 'Y'){
            ...
            //点击"确认预约"按钮
            $("#confirmBtn").Click(function(){
                if((data.payAppointment == 'on' || isCrossInner ) && !$.$.ua.isWX){
        ...
        if(itemData["technicians"] && itemData["technicians"].length>0){
            ...
            for(j=0;j<itemData["technicians"].length;j++){
                itemHtml += '<div class="list" hh="'+itemData["technicians"][j]["id"]+'">\
                                        ...
                                        <div>\
                                            <div><div>'+($.$.ua.isFirefox && itemData["technicians"][j]["name"].length>3 ? itemData["technicians"][j]["name"].substr(0,3)+"..." : itemData["technicians"][j]["name"])+'</div><div>[<span>'+(itemData["technicians"][j]["serialNo"] || '')+'</span>]</div> </div>\
